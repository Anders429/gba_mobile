...
impl Display for InvalidLength {
    fn fmt(&self, formatter: &mut Formatter) -> fmt::Result {
        match self {
            Self::IspLogin(length) => write!(
                formatter,
                "received length of {length} for {} packet, but expected length of 12",
                Command::IspLogin
            ),
    ...
    fn fmt(&self, formatter: &mut Formatter) -> fmt::Result {
        match self {
            ...
            Self::UnexpectedCommandError(_) => {
                formatter.write_str("received unexpected command error while logging in")
            }
        }
    }
}
