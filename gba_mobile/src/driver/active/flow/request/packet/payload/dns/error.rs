...
impl Display for InvalidLength {
    fn fmt(&self, formatter: &mut Formatter) -> fmt::Result {
        match self {
            Self::DnsQuery(length) => write!(
                formatter,
                "received length of {length} for {} packet, but expected length of 4",
                Command::DnsQuery
            ),
    ...
    fn fmt(&self, formatter: &mut Formatter) -> fmt::Result {
        match self {
            ...
            Self::UnexpectedCommandError(_) => {
                formatter.write_str("received unexpected command error while querying DNS")
            }
        }
    }
}
