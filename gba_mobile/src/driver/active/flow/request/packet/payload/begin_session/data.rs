use super::HANDSHAKE;
use core::{
    fmt,
    fmt::{Display, Formatter},
};
use deranged::RangedU8;

#[derive(Debug)]
pub(in crate::driver) enum Data {
    Byte0,
    Byte1,
    Byte2,
    Byte3,
    Byte4,
    Byte5,
    Byte6,
    Byte7,
}

impl Data {
    const BYTE0: u8 = HANDSHAKE[0];
    const BYTE1: u8 = HANDSHAKE[1];
    const BYTE2: u8 = HANDSHAKE[2];
    const BYTE3: u8 = HANDSHAKE[3];
    const BYTE4: u8 = HANDSHAKE[4];
    const BYTE5: u8 = HANDSHAKE[5];
    const BYTE6: u8 = HANDSHAKE[6];
    const BYTE7: u8 = HANDSHAKE[7];

    pub(super) fn new() -> Self {
        Self::Byte0
    }

    pub(super) fn receive_data(self, byte: u8) -> Result<Option<Self>, (Error, Option<u16>)> {
        match (self, byte) {
            (Self::Byte0, Self::BYTE0) => Ok(Some(Self::Byte1)),
            (Self::Byte1, Self::BYTE1) => Ok(Some(Self::Byte2)),
            (Self::Byte2, Self::BYTE2) => Ok(Some(Self::Byte3)),
            (Self::Byte3, Self::BYTE3) => Ok(Some(Self::Byte4)),
            (Self::Byte4, Self::BYTE4) => Ok(Some(Self::Byte5)),
            (Self::Byte5, Self::BYTE5) => Ok(Some(Self::Byte6)),
            (Self::Byte6, Self::BYTE6) => Ok(Some(Self::Byte7)),
            (Self::Byte7, Self::BYTE7) => Ok(None),
            (Self::Byte0, _) => Err((
                Error {
                    byte,
                    index: RangedU8::new_static::<0>(),
                },
                Some(1),
            )),
            (Self::Byte1, _) => Err((
                Error {
                    byte,
                    index: RangedU8::new_static::<1>(),
                },
                Some(2),
            )),
            (Self::Byte2, _) => Err((
                Error {
                    byte,
                    index: RangedU8::new_static::<2>(),
                },
                Some(3),
            )),
            (Self::Byte3, _) => Err((
                Error {
                    byte,
                    index: RangedU8::new_static::<3>(),
                },
                Some(4),
            )),
            (Self::Byte4, _) => Err((
                Error {
                    byte,
                    index: RangedU8::new_static::<4>(),
                },
                Some(5),
            )),
            (Self::Byte5, _) => Err((
                Error {
                    byte,
                    index: RangedU8::new_static::<5>(),
                },
                Some(6),
            )),
            (Self::Byte6, _) => Err((
                Error {
                    byte,
                    index: RangedU8::new_static::<6>(),
                },
                Some(7),
            )),
            (Self::Byte7, _) => Err((
                Error {
                    byte,
                    index: RangedU8::new_static::<7>(),
                },
                None,
            )),
        }
    }
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub(in crate::driver) struct Error {
    byte: u8,
    index: RangedU8<0, 7>,
}

impl Display for Error {
    fn fmt(&self, formatter: &mut Formatter) -> fmt::Result {
        write!(
            formatter,
            "unexpected byte {:#04x} at index {}; expected {:04x}",
            self.byte,
            self.index,
            HANDSHAKE[self.index.get() as usize]
        )
    }
}

impl core::error::Error for Error {}
