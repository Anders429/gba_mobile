...
enum State {
    ...
    OpenTcp(Packet<payload::OpenTcp>),
}

#[derive(Debug)]
pub(in super::super) struct OpenTcp {
...
impl OpenTcp {
    ...
    pub(super) fn with_socket_addr(
        ...
        Self {
            ...
            connection_generation,
            socket_generation,
            state: State::OpenTcp(Packet::new(
                payload::OpenTcp::new(addr),
                transfer_length,
                timer,
            )),
        }
    ...
    pub(super) fn vblank(self) -> Result<Self, Timeout> {
        match self.state {
            State::Dns { packet, port } => packet
                ...
                .map_err(Timeout::Dns),
            State::OpenTcp(packet) => packet
                .vblank()
                .map(|packet| Self {
                    socket_index: self.socket_index,
                    connection_generation: self.connection_generation,
                    socket_generation: self.socket_generation,
                    state: State::OpenTcp(packet),
                })
    ...
    pub(super) fn timer(&mut self) {
        match &mut self.state {
            ...
            State::OpenTcp(packet) => packet.timer(),
        }
    }
                        ...
                        *adapter = response.adapter;
                        match response.payload {
                            payload::dns::ReceiveParsed::Success(ip) => Some(Self {
                                ...
                                socket_generation: self.socket_generation,
                                state: State::OpenTcp(Packet::new(
                                    payload::OpenTcp::new(SocketAddrV4::new(ip, port)),
                                    transfer_length,
                                    timer,
                                )),
                            }),
                        ...
                        *adapter = response.adapter;
                        match response.payload {
                            ...
                            payload::open_tcp::ReceiveParsed::NotConnected => {
                                if let Some(socket_state) = socket_state {
                                    *socket_state =
                                        socket::State::Failure(socket::Failure::Connect);
                                }
                            }
                        }
                        None
